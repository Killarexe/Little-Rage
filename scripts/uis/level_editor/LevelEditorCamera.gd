...
func handle_camera_mouvement(delta: float) -> void:
  if brush.level.mode == LevelPlayer.Mode.PLAY:
    return
...
func handle_mobile_touch(delta: float) -> void:
  if touch_points.size() == 1 && touch_points.has(0) && !is_drag:
  ...
  elif (touch_points.size() != 1 || !touch_points.has(0)):
    long_press_timer = 0.0
    long_press_detected = false

func handle_desktop() -> void:
  if can_move:
...
func _process(delta: float) -> void:
  var is_mobile: bool = Game.is_mobile || debug_mode
  if is_mobile:
    handle_mobile_touch(delta)
  else:
    handle_desktop()
  handle_camera_mouvement(delta)
